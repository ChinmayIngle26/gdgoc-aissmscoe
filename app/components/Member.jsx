      ...
      <div className="relative border border-cream h-56 bg-black group overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-full flex justify-center gap-4 items-center transform translate-y-full group-hover:translate-y-0 group-focus-within:translate-y-0 transition-all duration-300 ease-in-out bg-gradient-to-b from-transparent via-purple/50 to-purple pb-3 pt-10">
          <div className="flex gap-4 justify-center">
            {member.github && (
              <Link href={member.github} target="_blank" rel="noopener noreferrer" aria-label={`${member.name}'s GitHub profile`}>
            ...
            {member.twitterinstagram && (
              <Link href={member.twitterinstagram} target="_blank" rel="noopener noreferrer" aria-label={`${member.name}'s ${member.twitterinstagram.includes("instagram") ? "Instagram" : "Twitter"} profile`}>
